...
const useAssignedStore = create(
  persist((set, get) => ({
    ...
    getAssignments: (category = '') =>
      get().assignments.filter((assigned) => (category ? assigned.category_key === category : true)),
    getAssigned: (id) => get().assignments.find((assigned) => id === assigned.id),
    setAssigned: (id) => set((state) => ({
    ...
    addNotification: (id) => set((state) => ({
      notifications: { ...state.notifications, [id]: state.assigned?.id !== id },
    })),
    fetchAssignments: async (workspace) => {
      ...
      try {
        ...
        const data = await apiWithAuth('get', `/api/workspaces/${workspace.id}/assignments/`)
        if (data) {
          const assignments = []
          let assigned = null
    ...
    addAssigned: async ({ title, description, category_key, stage_key, workspace, owner }) => {
      if (get().isLoading) {
        return
      }
      try {
    ...
    updateAssigned: async (workspace, id, { title, description, category_key, stage_key, owner }) => {
      if (get().isLoading) {
        return
      }
      try {
        ...
        set((state) => ({
          assignments: state.assignments.map((assigned) =>
            assigned.id === id ? { ...assigned, title, description, category_key, stage_key, owner } : assigned
          ),
        }))
        return data
      } catch (e) {
    ...
    deleteAssigned: async (workspace, id) => {
      ...
      try {
        ...
        set((state) => ({
          assignments: state.assignments.filter((assigned) =>
            assigned.id !== id
          ),
        }))
        return null
  ...
  }),
    {
      ...
      partialize: (state) => Object.fromEntries(
        Object.entries(state).filter(([key]) => !['isLoading, error, assigned'].includes(key)),
      ),
    }
  ),
)

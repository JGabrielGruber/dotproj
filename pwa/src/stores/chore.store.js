  ...
  persist((set, get) => ({
    ...
    fetchChores: async (workspace) => {
      ...
      try {
        ...
        const data = await apiWithAuth('get', `/api/workspaces/${workspace.id}/chores/`)
        if (data) {
          const chores = []
          let chore = null
    ...
    updateChore: async (workspace, id, { title, description, category_key, stage_key, owner }) => {
      if (get().isLoading) {
        return
      }
      try {
        ...
        set((state) => ({
          chores: state.chores.map((chore) =>
            chore.id === id ? { ...chore, title, description, category_key, stage_key, owner } : chore
          ),
        }))
        return data
      } catch (e) {
  ...
  }),
    {
      ...
      partialize: (state) => Object.fromEntries(
        Object.entries(state).filter(([key]) => !['isLoading, error, chore'].includes(key)),
      ),
    }
  ),
)
